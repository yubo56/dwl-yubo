...
enum { LyrBg, LyrBottom, LyrTile, LyrFloat, LyrTop, LyrFS, LyrBar, LyrOverlay, LyrBlock, NUM_LAYERS }; /* scene layers */
...
{
	/* Keep the complete window inside the usable area. */
	c->geom.width = MIN(bbox->width,
			MAX(1 + 2 * (int)c->bw, c->geom.width));
	c->geom.height = MIN(bbox->height,
			MAX(1 + 2 * (int)c->bw, c->geom.height));
	c->geom.x = MAX(bbox->x,
			MIN(c->geom.x, bbox->x + bbox->width - c->geom.width));
	c->geom.y = MAX(bbox->y,
			MIN(c->geom.y, bbox->y + bbox->height - c->geom.height));
}
...
buttonpress(struct wl_listener *listener, void *data)
{
	...
	case WL_POINTER_BUTTON_STATE_PRESSED:
		...
		if (!c && !exclusive_focus &&
			(node = wlr_scene_node_at(&layers[LyrBar]->node, cursor->x, cursor->y, NULL, NULL)) &&
...
createmon(struct wl_listener *listener, void *data)
{
	...
	m->scene_buffer = wlr_scene_buffer_create(layers[LyrBar], NULL);
...
destroynotify(struct wl_listener *listener, void *data)
{
	...
	if (c == exclusive_focus) {
		wlr_log(WLR_DEBUG, "destroyed XWayland exclusive focus %p", (void *)c);
...
focusclient(Client *c, int lift)
{
	...
	/* Explicit focus changes override XWayland popup focus. */
	if (lift && old_c && old_c == exclusive_focus && client_wants_focus(old_c)) {
		wlr_log(WLR_DEBUG, "explicit focus overrides XWayland exclusive focus %p", (void *)old_c);
		exclusive_focus = NULL;
	}
	...
	if (old && (!c || client_surface(c) != old)) {
		...
		if (old_client_type == LayerShell && wlr_scene_node_coords(
			...
			wlr_log(WLR_DEBUG, "XWayland exclusive focus %p blocked focus change", (void *)old_c);
			return;
...
mapnotify(struct wl_listener *listener, void *data)
{
	...
	if (client_is_unmanaged(c)) {
		...
		if (client_wants_focus(c)) {
			focusclient(c, 1);
			exclusive_focus = c;
			wlr_log(WLR_DEBUG, "XWayland client %p acquired exclusive focus", (void *)c);
		}
...
rendermon(struct wl_listener *listener, void *data)
{
	 ...
	 * this monitor. */
	wl_list_for_each(c, &clients, link) {
		if (c->resize && !c->isfloating && !c->isfullscreen
...
unmapnotify(struct wl_listener *listener, void *data)
{
	...
	if (client_is_unmanaged(c)) {
		if (c == exclusive_focus) {
			wlr_log(WLR_DEBUG, "unmapped XWayland exclusive focus %p", (void *)c);
			exclusive_focus = NULL;
			focusclient(focustop(selmon), 1);
		}
	} else {
